...
public class SelectStudentApp {
	public static void main(String[] args) {
		...
		try {
			...
			String sql="SELECT * FROM STUDENT ORDER BY NO"; //다중행은 ORDER BY 권장
			...
			// => false : ResultSet 커서 위치에 처리행이 존재하지 않는 경우의 반환값 - EOF(End Of File)
			// => true : ResultSet 커서 위치에 처리행이 존재하는 경우의 반환값
			if(rs.next()) {
...
//					int no=rs.getInt(1);
					int no=rs.getInt("no"); //권장(컬럼명을 확인할 수 있어 가독성 높음)
					String name=rs.getString("name");
					String phone=rs.getString("phone");
					String address=rs.getString("address");
//					Date birthday=rs.getDate("birthday"); //java.sql.Date 사용
					//ResultSet 커서 위치의 처리행의 컬럼값은 오라클 자료형에 상관없이
