...
--문제5.부서 테이블,사원 테이블, 급여 등급 테이블에서 사번,사원명,부서명,급여,등급을 검색하시오. 
...
--EMP 테이블에서 사원번호가 7521인 사원과 같은 업무를 하는 사원 중 사원번호가 7900인 사원보다 많은 급여를 받는 사원의
--사원번호,사원이름,업무,급여 검색
...
--EMP 테이블에서 SALES 부서에 근무하는 사원 중 급여를 가장 많는 사원의 사원번호,사원이름,업무,급여 검색
...
SELECT EMPNO,ENAME,SAL,DEPTNO FROM EMP WHERE SAL=(SELECT MIN(SAL) FROM EMP GROUP BY DEPTNO);--에러

--서브쿼리의 검색 결과값이 다중행인 경우 = 연산자 대신 IN 연산자를 사용하여 컬럼값 비교
...
--EMP 테이블에서 부서번호가 10인 부서에 근무하는 어떤 사원보다 급여가 적은 사원의 사원번호,사원이름,급여,부서번호 검색
...
--다중행 서브쿼리의 ANY 또는 ALL 대신 단일행 서브쿼리의 MIN 또는 MAX 함수 사용
...
--서브쿼리의 검색대상이 여러 개(MULTI-COLUMN SUBQUERY)인 경우 비교할 컬럼을 () 안에 ,로 구분하여 나열하면 비교 가능   
...
--집합연신자를 사용할 경우 두 개의 SELECT 명령의 검색대상은 갯수와 자료형이 반드시 동일하도록 작성
...
--집합연산자 사용시 두 개의 SELECT 명령에 대한 검색대상의 갯수 또는 자료형이 다른 경우 에러 발생
...
--테이블에 행 삽입시 컬럼값을 저장하고 싶지 않은 경우 NULL를 전달하여 저장
INSERT INTO DEPT VALUES(70,'영업부',NULL);--명시적 NULL 저장
COMMIT;
SELECT * FROM DEPT;
...
UPDATE EMP SET DEPTNO=35 WHERE EMPNO=9000;--FK 제약조건을 위반하여 에러 발생
