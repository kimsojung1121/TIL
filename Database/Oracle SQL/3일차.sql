...
--동등조인(EQUI JOIN) : 교차조인된 검색결과에서 조인조건에 = 연산자를 사용하여 결과가 참인 행만 검색
...
--테이블 별칭(TABLE ALIAS) : 조인 테이블에 새로운 이름을 일시적으로 부여하는 기능
...
--비동등 조인(NON-EQUI JOIN) : 교차조인된 검색결과에서 조인조건에 = 연산자를 제외한 연산자를 사용하여 결과가 참인 행만 검색
...
--외부조인(OUTER JOIN) : 조인조건이 참인 행만 검색하는 것이 아니라 조인조건에 누락된 행도 검색
--조인조건의 테이블에 (+)를 사용하여 조인조건에 누락된 행의 컬럼값을 NULL로 검색되도록 설정

--EMP 테이블에서 모든 사원의 부서번호를 중복되지 않는 유일한 값으로 검색
SELECT DISTINCT DEPTNO FROM EMP; --검색결과 : 10,20,30

--DEPT 테이블에서 모든 부서의 부서번호 검샛
SELECT DEPTNO FROM DEPT; --검색결과 : 10,20,30,40

--EMP 테이블과 DEPT 테이블에서 모든 사원의 사원번호,사원이름,급여,부서이름,부서위치 검색
--조인조건 : EMP 테이블의 부서번호와 DEPT 테이블의 부서번호가 같은 행만 결합되어 검색
...
--사원이 없는 부서를 검색해야 되는 경우 EMP 테이블의 조인조건에 (+)를 붙여 EMP 테이블의 컬럼값 대신 NULL이 검색되도록 설정
SELECT EMPNO,ENAME,SAL,DNAME,LOC FROM EMP,DEPT WHERE EMP.DEPTNO(+)=DEPT.DEPTNO;

--셀프조인(SELF JOIN) : 하나의 테이블에 다른 여러 개의 별칭을 설정하여 조인하는 기능

--EMP 테이블 모든 사원의 사원번호,사원이름,매니저번호(매니저의 사원번호) 검색
SELECT EMPNO,ENAME,MGR FROM EMP;

--EMP 테이블에서 모든 사원의 사원번호,사원이름,매니저번호(매니저의 사원번호),매니저 이름(매니저의 사원이름) 검색
--조인조건 : EMP 테이블(WORKER)의 매니저번호를 EMP 테이블(MANAGER)의 사원번호와 비교하여 같은 행만 결합하여 검색
--매니저번호가 NULL인 사원 미검색
...
--매니저번호가 NULL인 사원도 검색하고자 할 경우 외부조인(OUTER JOIN) 이용
SELECT WORKER.EMPNO,WORKER.ENAME WORKER_ENAME,WORKER.MGR,MANAGER.ENAME MANAGER_ENAME FROM EMP WORKER,EMP MANAGER WHERE WORKER.MGR=MANAGER.EMPNO(+);
...
--조인 테이블에 같은 이름의 컬럼이 여러 개 있는 경우 JOIN~USING 구문을 이용하여 같은 이름의 컬럼값을 비교하여 조인 조인 - 조인조건을 명시하지 않고 조인
...
--형식)SELECT 검색대상,... FROM 테이블명1 {LEPT|RIGHT|FULL} [OUTER] JOIN 테이블명2 ON 조인조건
--LEPT OUTER JOIN : 왼쪽 방향에 선언된 테이블의 모든 행을 검색하되, 
--  오른쪽 방향에 선언된 테이블은 조인조건이 참인 행과 결합하여 검색하고 조인조건에 참인 행이 없는 경우 NULL로 결합하여 검색
--RIGHT OUTER JOIN : 오른쪽 방향에 선언된 테이블의 모든 행을 검색하되, 
--  왼쪽 방향에 선언된 테이블은 조인조건이 참인 행과 결합하여 검색하고 조인조건에 참인 행이 없는 경우 NULL로 결합하여 검색
--FULL OUTER JOIN : 양쪽 방향에 선언된 테이블의 모든 행을 검색하되, 
--  조인조건이 참인 행은 결합하여 검색하고 조인조건이 참인 행이 없는 경우 NULL로 결합하여 검색
...
--EMP 테이블에서 모든 사원의 사원번호,사원이름,매니저번호(매니저의 사원번호),매니저이름(매니저의 사원이름) 검색
--조인조건 : EMP 테이블(WORKER)의 매니저번호를 EMP 테이블(MANAGER)의 사원번호와 비교하여 같은 행만 결합하여 검색 - INNER JOIN
SELECT WORKER.EMPNO,WORKER.ENAME WORKER_MGR,MANAGER.ENAME MANAGER_ENAME     
    FROM EMP WORKER JOIN EMP MANAGER ON WORKER.MGR=MANAGER.EMPNO;

--OUTER JOIN을 사용하여 모든 사원 검색
SELECT WORKER.EMPNO,WORKER.ENAME WORKER_MGR,MANAGER.ENAME MANAGER_ENAME 
    FROM EMP WORKER LEFT JOIN EMP MANAGER ON WORKER.MGR=MANAGER.EMPNO;
    
    
--SQL:1999 JOIN 실습문제
--1. 부서 테이블과 사원 테이블에서 사번,사원명,부서코드,부서명을 검색하시오. (사원명은 오름차순 정렬할 것)
