...
--단순뷰 : 뷰를 이용하여 행 삽입,변경,삭제,검색 명령 사용 가능
...
--단순뷰를 이용한 행 삽입 - 테이블에 행 삽입 : 뷰에 없는 생략된 컬럼에는 컬럼 기본값이 저장
...
--복합뷰 : 행 검색 명령만 사용 가능 - 보다 쉬운 검색 기능 제공
...
--인사부에 근무하는 사원에게는 EMP_VIEW01 뷰에 접근할 수 있는 권한 부여
...
--EMP_VIEW20 뷰에서 사원번호가 7782인 사원정보를 삭제
...
--EMP_VIEW20 뷰에서 사원번호가 7788인 사원번호를 삭제
...
--정렬 전에 검색행에 행번호가 제공된 후 정렬되어 검색되므로 행번호가 비순차적
SELECT ROWNUM,EMPNO,ENAME,SAL FROM EMP ORDER BY SAL DESC; --처리 순서: FROM -> SELECT -> ORDER BY

--ROWNUM 키워드 대신 ROW_NUMBER() 순위함수를 이용하여 행번호를 반환받아 검색 - OVER() 분석함수 사용 --3일차.SQL 참고
SELECT ROW_NUMBER() OVER(ORDER BY SAL DESC) NUM,EMPNO,ENAME,SAL FROM EMP;

--EMP 테이블에서 모든 사원의 사원번호,사원이름,급여를 급여로 내림차순 정렬하여 검색하되, 행번호를 제공하여 검색하고, 검색된 행번호가 5보다 작은 행 검색
--WINDOW 함수(그룹함수,순위함수,순서함수 등)를 WHERE 구문에서 사용할 경우 에러 발생
...
--EMP_VIEW 뷰에 행번호를 제공하여 검색하되, 행번호가 5보다 작은 행만 검색
...
--EMP 테이블에서 모든 사원의 사원번호,사원이름,급여를 급여로 내림차순 정렬하여 검색하되, 검색행에 행번호를 제공하여 검색하고, 검색된 행번호가 5보다 작은 행만 검색
SELECT ROWNUM,TEMP.* FROM (SELECT EMPNO,ENAME,SAL FROM EMP ORDER BY SAL DESC) TEMP WHERE ROWNUM<5;

--EMP 테이블에서 모든 사원의 사원번호,사원이름,급여를 급여로 내림차순 정렬하여 검색하되, 검색행에 행번호를 제공하여 검색하고, 검색된 행번호가 10인 행만 검색
--WHERE 구문에서 행번호를 조건식에서 사용할 경우 조건식의 연산자가 < 또는 <= 연산자는 사용 가능하지만 나머지 비교 연산자를 사용할 경우 검색 불가능
--행번호는 행에 순차적으로 제공되므로 현재 행번호와 같거나 큰 행은 미검색
...
--EMP 테이블에서 모든 사원의 사원번호,사원이름,급여를 급여로 내림차순 정렬하여 검색하되, 검색행에 행번호를 제공하여 검색하고, 검색된 행번호가 5부터 10 범위의 행만 검색
SELECT * FROM (SELECT ROWNUM RN,TEMP.* FROM (SELECT EMPNO,ENAME,SAL FROM EMP ORDER BY SAL DESC) TEMP) WHERE RN BETWEEN 5 AND 10;

--EMP 테이블에서 모든 사원의 사원번호,사원이름,검색행 직전의 사원번호와 사원이름,검색행 직후행의 사원번호와 사원이름 검색
--사원번호로 오름차순 정렬하여 검색하되, 직전행 또는 직후행이 없는 경우 사원번호는 0, 사원이름은 NULL로 검색
SELECT EMPNO,ENAME,LAG(EMPNO,1,0) OVER(ORDER BY SAL) BEFORE_EMPNO,LAG(EMPNO,1,NULL) OVER(ORDER BY SAL) BEFORE_ENAME
    ,LEAD(EMPNO,1,0) OVER(ORDER BY SAL) AFTER_EMPNO,LEAD(EMPNO,1,NULL) OVER(ORDER BY SAL) AFTER_ENAME
    FROM EMP;

--EMP 테이블에서 사원번호가 7844인 사원의 사원번호,사원이름,검색행 직전의 사원번호와 사원이름,검색행 직후행의 사원번호와 사원이름 검색
--사원번호로 오름차순 정렬하여 검색하되, 직전행 또는 직후행이 없는 경우 사원번호는 0, 사원이름은 NULL로 검색
--WHERE 구문이 먼저 실행되어 검색되므로 직전행과 직후행이 존재하지 않아 직전행과 직후행의 컬럼값 대신 0과 NULL이 검색
SELECT EMPNO,ENAME,LAG(EMPNO,1,0) OVER(ORDER BY SAL) BEFORE_EMPNO,LAG(EMPNO,1,NULL) OVER(ORDER BY SAL) BEFORE_ENAME
    ,LEAD(EMPNO,1,0) OVER(ORDER BY SAL) AFTER_EMPNO,LEAD(EMPNO,1,NULL) OVER(ORDER BY SAL) AFTER_ENAME
    FROM EMP WHERE EMPNO=7844;

--인라인 뷰를 이용하여 모든 사원을 검색한 후 원하는 행에 대한 검색 결과 제공
SELECT * FROM (SELECT EMPNO,ENAME,LAG(EMPNO,1,0) OVER(ORDER BY SAL) BEFORE_EMPNO,LAG(EMPNO,1,NULL) OVER(ORDER BY SAL) BEFORE_ENAME
    ,LEAD(EMPNO,1,0) OVER(ORDER BY SAL) AFTER_EMPNO,LEAD(EMPNO,1,NULL) OVER(ORDER BY SAL) AFTER_ENAME
    FROM EMP) WHERE EMPNO=7844;
--EX)게시판 게시글의 현재글, 이전글, 다음글을 한번에 검색할 때 사용

--시퀀스(SEQUENCE) : 숫자값(정수)을 저장하여 자동 증가되는 값을 제공하는 객체

--시퀀스 생성
--형식)CREATE SEQUENCE 시퀀스명 [START WITH 초기값] [INCREMENT BY 증가값] [MAXVALUE 최댓값] [MINVALUE 최솟값]
--    [CYCLE] [CACHE 개수]
--START WITH 초기값 : 시퀀스 객체에 저장된 초기값 설정 - 생략 : NULL
--INCREMENT BY 증가값 : 자동 증가되는 숫자값 설정 - 생략 : 1
--MAXVALUE 최댓값 : 시퀀스 객체에 저장할 수 있는 최댓값 설정 - 생략 : 숫자값으로 표현될 수 있는 최댓값 
--MINVALUE 최솟값 : 시퀀스 객체에 저장할 수 있는 최솟값 설정 - 생략 : 1
--CYCLE : 시퀀스 객체에 저장값이 최댓값을 초과한 경우 최솟값부터 다시 제공되도록 반복하는 기능
--CACHE 개수 : 임시 저장공간에 자동 증가값을 미리 생성하여 제공할 수 있는 개수를 설정하는 기능 제공 - 생략 : 20

--USER2 테이블 생성 : 번호(숫자형),이름(문자형),생년월일(날짜형)
...
--시퀀스에 NULL이 저장된 경우 시퀀스의 최솟값을 제공하고 시퀀스는 제공된 숫자값으로 자동 변경
--시퀀스에 숫자값이 저장된 경우 증가값만큼 더해진 숫자값을 제공하고 시퀀스는 제공된 숫자값으로 자동 변경
SELECT USER2_SEQ.NEXTVAL FROM DUAL; --> 최솟값 1 검색되고 시퀀스 숫자값 변경 : 1
SELECT USER2_SEQ.CURRVAL FROM DUAL; --> 시퀀스 현재값 : 1
SELECT USER2_SEQ.NEXTVAL FROM DUAL; --> 증가값 1이 더해져 2 검색되고 시퀀스 숫자값 변경 : 2

--USER2 테이블에 행 삽입 - 시퀀스의 자동 증가값을 NO 컬럼에 전달하여 저장
...
--조건식에서 많이 사용되는 컬럼에 설정하여 행이 많을 때 설정하는 것이 효율적
...
CREATE TABLE USER3(NO NUMBER(2) CONSTRAINT USER3_NO_PK PRIMARY KEY,NAME VARCHAR2(20),EMAIL VARCHAR2(50) CONSTRAINT USER3_EMAIL_UK UNIQUE);
DESC USER3;
...
SELECT C.INDEX_NAME,COLUMN_NAME,UNIQUENESS,INDEX_TYPE FROM USER_INDEXES I JOIN USER_IND_COLUMNS C ON I.INDEX_NAME=C.INDEX_NAME WHERE C.TABLE_NAME='USER3';

--인덱스(NON-UNIQUE INDEX) 생성
--형식)CREATE INDEX 인덱스명 ON 테이블명(컬럼명)
--저장행이 많은 테이블의 조건에서 사용되는 컬럼에 설정
...
--형식)DROP INDEX 인덱스명
--UNIQUE INDEX는 제약조건을 삭제하면 자동으로 삭제 처리
...
--USER3 테이블의 NAME 컬럼에 설정된 인덱스 삭제 - NON-UNIQUE INDEX
DROP INDEX USER3_NAME_INDEX;
--인덱스 확인
SELECT C.INDEX_NAME,COLUMN_NAME,UNIQUENESS,INDEX_TYPE FROM USER_INDEXES I JOIN USER_IND_COLUMNS C ON I.INDEX_NAME=C.INDEX_NAME WHERE C.TABLE_NAME='USER3';

--동의어(SYNONYM) : 오라클 객체에 별명을 부여하여 사용하는 객체
--전용 동의어 : 특정 사용자마나 사용할 수 있는 동의어 - 일반 사용자 관리
...
--테이블 목록 확인 - SYS.USER_TABLES : SYS 사용자 스키마
SELECT TABLE_NAME FROM SYS.USER_TABLES;
SELECT TABLE_NAME FROM USER_TABLES; --SYS.USER_TABLES 객체(테이블)의 동의어 : USER_TABLES
SELECT TABLE_NAME FROM TABS; --SYS.USER_TABLES 객체(테이블)의 동의어 : TABS

--EMP 테이블에서 성과급이 NULL이 아닌 사원의 사원이름,업무,급여,성과급을 검색하여 COMM 테이블에 삽입
DESC COMM;
SELECT * FROM COMM;
...
--시스템 관리자는(SYS 또는 SYSTEM)로 접속하여 일반 사용자(SCOTT)에게 동의어에 대한 시스템 권한 부여
--GRANT CREATE SYNONYM TO SCOTT;

--관리자로부터 동의어에 대한 시스템 권한을 부여받은 후 동의어 관련 명령 사용 가능
CREATE SYNONYM BONUS FOR COMM;
SELECT * FROM COMM;
SELECT * FROM BONUS;
...
SELECT OWNER,TABLE_NAME,SYNONYM_NAME,TABLE_OWNER FROM SYS.ALL_SYNONYMS WHERE TABLE_NAME='DUAL';

SELECT TABLE_NAME FROM SYS.USER_TABLES;
SELECT TABLE_NAME FROM USER_TABLES;
SELECT TABLE_NAME FROM TABS;
SELECT OWNER,TABLE_NAME,SYNONYM_NAME,TABLE_OWNER FROM SYS.ALL_SYNONYMS WHERE TABLE_NAME='USER_TABLES';
...
SELECT OWNER,TABLE_NAME,SYNONYM_NAME,TABLE_OWNER FROM SYS.ALL_SYNONYMS WHERE TABLE_NAME='EMP';
SELECT * FROM SCOTT_EMP; --에러 발생

--현재 사용자의 BONUS 전용 동의어 삭제
SELECT OWNER,TABLE_NAME,SYNONYM_NAME,TABLE_OWNER FROM SYS.ALL_SYNONYMS WHERE TABLE_NAME='COMM';
DROP SYNONYM BONUS;
