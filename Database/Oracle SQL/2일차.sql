...
--ROUND(숫자값,소수점자릿수) : 숫자값을 전달받아 소수점 자리수만큼 검색되도록 반올림 처리하여 반환하는 함수
--DUAL : 검색 테이블 없이 SELECT 명령을 작성할 경우 사용되는 가상의 테이블
SELECT ROUND(45.582,2),ROUND(45.582,0),ROUND(45.582,-1) FROM DUAL;

--TRUNC(숫자값,소수점자릿수) : 숫자값을 전달받아 소수점 자릿수만큼 검색되도록 절삭 처리하여 반환하는 함수
SELECT TURNC(45.582,2),TRUNC(45.582,0),TRUNC(45.582,-1) FROM DUAL;

--CELL(숫자값) : 숫자값을 전달받아 소수점 이하 값이 존재할 경우 증가된 정수값으로 반환하는 함수
SELECT CELL(15.3),CELL(-15.3) FROM DUAL;

--FLOOR(숫자값) : 숫자값을 전달받아 소수점 이하 값이 존재할 경우 감소된 정수값으로 반환하는 함수
...
--ADD_MONTHS(날짜값,숫자값) : 날짜값을 전달받아 숫자값만큼의 개월수를 더한 날짜값을 변환하는 함수
...
--TRUNC(날짜값,표현단위) : 날짜값을 전달받아 원하는 단위까지만 표현하고 나머지는 최소값으로 변환하여 반환하는 함수
...
--날짜 패턴에 맞지 않는 문자값을 사용한 경우 에러 발생
SELECT EMPNO,ENAME,HIREDATE FROM EMP WHERE HIREDATE='01-23-1982'; --에러 발생
--TO_DATE 함수의 패턴문자를 이용하면 원하는 패턴 형식으로 날짜값 표현 가능 - TO_DATE(문자값,패턴문자)
...
--TO_DATE 함수를 사용하여 문자값을 날짜값으로 변환하여 연산 처리
...
SELECT EMPNO,ENAME,TO_CHAR(SAL,'$999,990.00') SAL FROM EMP WHERE EMPNO=7844;
...
SELECT EMPNO,ENAME,(SAL_COMM)*12 ANNUAL FROM EMP; --검색 오류
--NVL 함수를 사용하여 성과급이 NULL인 경우 0으로 변환하여 연산 처리
SELECT EMPNO,ENAME,(SAL+NVL(COMM,0))*12 ANNUAL FROM EMP;

--NVL2(컬럼명,변경값1,변경값2) : 컬럼값이 NULL이 아닌 경우 변경값1로 변환하고 NULL인 경우 변경값2로 변환하여 반환하는 함수
...
SELECT EMPNO,ENAME,DECODE(JOB,'ANALYST',SAL) "ANALYST", DECODE(JOB,'CLERK',SAL) "CLERK",
    DECODE(JOB,'MANAGER',SAL) "MANAGER", DECODE(JOB,'PRESIDENT',SAL) "PRESIDENT", DECODE(JOB,'SALESMAN',SAL) "SALESMAN"
    FROM EMP;
    
--그룹함수 : 값을 여러개 전달받아 처리하여 결과값을 반환하는 함수

--COUNT(컬럼명) : 컬럼값의 개수를 반환하는 함수 - 검색행의 개수 반환
SELECT COUNT(EMPNO) FROM EMP;

--그룹함수는 다른 검색대상과 같이 사용 불가능 - 검색대상의 검색행의 개수가 다르므로 에러 발생
SELECT EMPNO,COUNT(EMPNO) FROM EMP; --에러 발생

--그룹함수는 NULL을 값으로 인식하지 않아 미처리
SELECT COUNT(EMPNO),COUNT(COMM) FROM EMP;

--COUNT 함수는 컬럼명 대신 [*]를 사용하여 모든 컬럼으로 테이블에 저장된 행의 개수 반환
SELECT COUNT(*) FROM EMP;

--MAX(컬럼명) : 컬럼값 중 최댓값을 반환하는 함수
SELECT MAX(SAL) FROM EMP;
SELECT MAX(ENAME) FROM EMP;
SELECT MAX(HIREDATE) FROM EMP;

--MIN(컬럼명) : 컬럼값 중 최솟값을 반환하는 함수
...
SELECT DEPTNO DNO,COUNT(*) FROM EMP GROUP BY DNO ORDER BY DEPTNO; --순서: FROM -> GROUP BY -> SELECT -> ORDER BY
SELECT DEPTNO DNO,COUNT(*) FROM EMP GROUP BY DEPTNO ORDER BY DNO;
...
SELECT JOB,CEIL(AVG(SAL)) AVG_SAL FROM EMP WHERE JOB !='PRESIDENT' GROUP BY JOB; --PRESIDENT가 많을 때
SELECT DEPTNO,SUM(SAL) FROM EMP GROUP BY JOB HAVING JOB !='PRESIDENT'; --PRESIDENT가 적을 때
...
SELECT DEPTNO,SUM(SAL) FROM EMP WHERE SUM(SAL)>=9000 GROUP BY DEPTNO; --에러


--단일 함수
--문제 1. 사원테이블에서 입사일이 12월인 사원의 사번,사원명,입사일 검색하시오
SELECT EMPNO,ENAME,HIREDATE FROM EMP WHERE TO_CHAR(HIREDATE,'MM')=12;

--문제 2. 다음과 같은 결과를 검색할 수 있는 SQL 문장을 작성하시오.
SELECT EMPNO,ENAME,LPAD(SAL,10,'*') "급여" FROM EMP;

--문제 3. 다음과 같은 결과를 검색할 수 있는 SQL 문장을 작성하시오.
SELECT EMPNO,ENAME,TO_CHAR(HIREDATE,'YYYY-MM-DD') "입사일" FROM EMP;


--그룹 함수
--문제 1. 사원테이블에서 부서별 인원수가 6명 이상인 부서코드를 검색할 수 있는 SQL 문장을 작성하시오.
SELECT DEPTNO, COUNT(*) FROM EMP GROUP BY DEPTNO HAVING COUNT(*)>=6;

--문제 2. 사원테이블로부터 부서번호, 업무별 급여합계를 계산하고자 한다. 다음과 같은 결과를 출력할 수 있는 SQL문장을 작성하시오.
SELECT DEPTNO,DECODE(JOB,'MANAGER',SUM(SAL)) FROM EMP;

--문제 3. 사원테이블로부터 년도별, 월별 급여합계를 출력할 수 있는 SQL문장을 작성하시오
SELECT TO_CHAR(HIREDATE,'YYYY') "년",TO_CHAR(HIREDATE,'MM') "월",SUM(SAL) FROM EMP GROUP BY HIREDATE ORDER BY HIREDATE;

--문제 4. 사원테이블에서 부서별 COMM(커미션)을 포함하지 않은 연봉의 합과 포함한 연봉의 합을 구하는 SQL을 작성하시오.
...
--문제 5. 사원테이블에서 SALESMAN을 제외한 JOB별 급여합계
