...
//다중 스레드 프로그램 - main 스레드 외 다른 스레드를 생성하여 새로운 명령 실행
...
//		thread.start();
		...
		// => Thread 인스턴스로 start() 메소드 호출
		// => 생성된 스레드를 만들어 run() 메소드의 명령 실행
		...
		*/
		...
		thread.start();
		*/
		//Thread 인스턴스로 start() 메소드 외 호출할 메소드가 없는 경우 참조변수 없이 메소드 호출
		...
		*/
		...
		for(char i='0';i<='9';i++) {
			System.out.print(i);
			Thread.sleep(500);
		}//main 스레드가 소멸된다고 해서 프로그램이 종료되는 것이 아님
